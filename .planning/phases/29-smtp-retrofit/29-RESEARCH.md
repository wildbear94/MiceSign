...
1. **CONTEXT D-D3 "GreenMail 1.6.x" 는 오기입인가 실제 의도인가?**
   ...
   - **RESOLVED:** CONTEXT.md D-D3 2026-04-23 업데이트로 GreenMail 2.1.x 상향 확정 (user 승인 완료). Plan 04 Task 1 에서 `com.icegreen:greenmail-junit5:2.1.3` 로 버전 pinning — Jakarta Mail 2.x / Spring Boot 3.5 호환.
...
2. **`@Lazy` self-injection vs `TransactionTemplate`:**
   ...
   - **RESOLVED:** Plan 03 Task 2 에서 `@Lazy ApprovalEmailSender self` 생성자 주입 패턴 채택 (Spring 공식 권장안 + project convention 일관성). `TransactionTemplate` 은 fallback 으로만 보류.
...
3. **`ApprovalEmailSender.send()` `@Retryable` 재시도 간격 5분 이 실제 테스트에서 허용되는가?**
   ...
   - **RESOLVED:** Plan 03 Task 2 action/verify 에서 `@Backoff(delayExpression = "#{${app.mail.retry.delay-ms:300000}}")` 로 처음부터 통일 (default 5분 = 300000ms 유지). Plan 01 Task 3 에서 `src/main/resources/application.yml` 에 `app.mail.retry.delay-ms: 300000` default property 추가. Plan 04 Task 3 는 `src/test/resources/application-test.yml` 에 `app.mail.retry.delay-ms: 0` override 만 수행하여 테스트가 backoff 없이 즉시 완료되도록 함.
...
4. **MailHog vs Mailpit 선택 (STATE.md Blocker):**
   ...
   - **RESOLVED:** Phase 33 개발자 런북으로 이관 — 기능 동등, Phase 29 스코프 밖. Phase 29 의 자동 테스트는 GreenMail 2.1.3 (in-JVM) 로 완결.
...
5. **운영 SMTP 공급자 (STATE.md Blocker):**
   ...
   - **RESOLVED:** Phase 33 운영 전환 런북으로 이관 — 회사 IT 정책 결정 대기. Phase 29 는 `MAIL_HOST`/`MAIL_USERNAME`/`MAIL_PASSWORD`/`APP_BASE_URL` env var agnostic 으로 완결 (Plan 05 Task 1 `application-prod.yml` + user_setup 명시).
